...
所有的[`markdown` 配置选项](/zh-cn/reference/configuration-reference/#markdown-选项)可以在 MDX 集成中单独配置。这包括备注和 rehype 插件，语法高亮，等等。选项将默认为你的 Markdown 配置中的选项（[查阅 `extendMarkdownConfig` 选项](#extendmarkdownconfig)来进行修改）。
